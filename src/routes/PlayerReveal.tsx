  ...
  return (
    ...
    >
      <div className="flex flex-col gap-5 pt-2">
        ...
        {isTeamMode && teams.length > 0 && (
          ...
          >
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-xs font-bold px-2 py-0.5 rounded-full bg-neon-green/15 text-neon-green">LIVE</span>
            </div>
              ...
              <>
                <div className="flex items-center justify-between mb-3">
                  <div className="text-center flex-1">
                    <p className="font-score text-3xl font-bold" style={{ color: teams[0].colour }}>{formatRands(teams[0].score)}</p>
                  ...
                  <div className="text-center flex-1">
                    <p className="font-score text-3xl font-bold" style={{ color: teams[1].colour }}>{formatRands(teams[1].score)}</p>
                ...
                <div className="flex h-2.5 rounded-full overflow-hidden bg-bg-elevated">
                  <div className="transition-all duration-700 rounded-l-full" style={{ width: `${Math.max(5, (teams[0].score / totalTeamScore) * 100)}%`, backgroundColor: teams[0].colour }} />
                  <div className="transition-all duration-700 rounded-r-full" style={{ width: `${Math.max(5, (teams[1].score / totalTeamScore) * 100)}%`, backgroundColor: teams[1].colour }} />
                </div>
              </>
            ) : (
              <div className="flex flex-col gap-2">
                {[...teams].sort((a, b) => b.score - a.score).map((team, idx) => (
                  <div key={team.id} className="flex items-center gap-3">
                    ...
                    <span className="font-score text-sm" style={{ color: team.colour }}>{formatRands(team.score)}</span>
                  </div>
                ))}
              </div>
            )}
        ...
        {/* Personal result */}
        <motion.div
          className={`rounded-2xl p-6 flex flex-col items-center gap-3 ${isCorrect ? 'bg-neon-green/10 border border-neon-green/30' : 'bg-neon-pink/10 border border-neon-pink/30'}`}
          initial={{ scale: 0.9, opacity: 0 }}
          animate={{ scale: 1, opacity: 1 }}
          transition={{ delay: 0.1, type: 'spring', stiffness: 240, damping: 20 }}
        >
          <span className="text-5xl">{isCorrect ? '✅' : '❌'}</span>
          <p className={`font-display text-3xl tracking-wide ${isCorrect ? 'text-neon-green' : 'text-neon-pink'}`}>
            {isCorrect ? 'CORRECT!' : 'INCORRECT'}
          </p>
          {me && <p className="font-score text-2xl text-neon-gold font-bold">{formatRands(me.score)}</p>}
        </motion.div>

        {/* Correct answer */}
        {reveal && (
          <motion.div className="bg-bg-card shadow-soft rounded-2xl p-5 flex flex-col gap-2" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
            ...
            {reveal.explanation && <p className="text-sm text-text-secondary leading-relaxed">{reveal.explanation}</p>}
          </motion.div>
        )}

        {/* Rankings */}
        <motion.div className="bg-bg-card shadow-soft rounded-2xl p-5 flex flex-col gap-4" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
          <p className="text-xs text-text-muted tracking-[0.15em] uppercase">{isTeamMode ? 'Round Rankings' : 'Scores'}</p>
          <div className="flex flex-col gap-2">
            {sortedPlayers.map((p, idx) => {
              ...
              return (
                <motion.div
                  key={p.id}
                  className={`flex items-center gap-3 px-3 py-2.5 rounded-xl ${isMe ? 'bg-bg-elevated' : ''}`}
                  initial={{ opacity: 0, x: -10 }}
                  animate={{ opacity: 1, x: 0 }}
                  transition={{ delay: 0.35 + idx * 0.05 }}
                >
                  <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${idx === 0 ? 'bg-neon-gold/20 text-neon-gold' : idx === 1 ? 'bg-text-secondary/15 text-text-secondary' : idx === 2 ? 'bg-neon-pink/15 text-neon-pink' : 'bg-bg-elevated text-text-muted'}`}>
                    {idx + 1}
                  </div>
                  <span className="text-xl">{p.avatar}</span>
                  {pTeam && <div className="w-2 h-2 rounded-full flex-shrink-0" style={{ backgroundColor: pTeam.colour }} />}
                  ...
                  <span className="font-score text-neon-gold text-sm">{formatRands(p.score)}</span>
                </motion.div>
              );
            })}
          </div>
