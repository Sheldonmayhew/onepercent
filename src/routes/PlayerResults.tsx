...
export function Component() {
  ...
  return (
    <GameLayout
      ...
      cta={<NavCTA onClick={handleLeave} variant="muted">LEAVE GAME</NavCTA>}
    >
      <div className="flex flex-col gap-5 pt-2">
        ...
        {me && myRank > 0 && (
          ...
          >
            <span className="font-display text-5xl text-neon-cyan w-16 text-center">#{myRank}</span>
          ...
          <div className="bg-bg-card shadow-soft rounded-2xl p-5">
            ...
            <div className="flex flex-col gap-2">
              {[...teams].sort((a, b) => b.score - a.score).map((team, idx) => {
                ...
                return (
                  <div key={team.id} className={`px-3 py-2.5 rounded-xl ${myTeam ? 'bg-bg-elevated' : ''}`}>
          ...
          <div className="flex flex-col gap-2">
            {sorted.map((player, idx) => {
              ...
              return (
                <motion.div
                  key={player.id}
                  className={`flex items-center gap-3 px-3 py-2.5 rounded-xl ${player.id === playerId ? 'bg-bg-elevated' : ''}`}
                  initial={{ opacity: 0, x: -10 }}
                  animate={{ opacity: 1, x: 0 }}
                  transition={{ delay: 0.35 + idx * 0.05 }}
                >
                  <span className={`font-score text-lg w-6 text-center ${idx === 0 ? 'text-neon-gold' : 'text-text-muted'}`}>
                    {idx === 0 ? '🥇' : idx === 1 ? '🥈' : idx === 2 ? '🥉' : `${idx + 1}`}
                  </span>
                  <span className="text-2xl">{player.avatar}</span>
                  {isTeamMode && team && <div className="w-2.5 h-2.5 rounded-full flex-shrink-0" style={{ backgroundColor: team.colour }} title={team.name} />}
                  <span className="flex-1 font-medium text-text-primary">
                    {player.name}
