...
export default function TvPlay({ question, players, roundState, timerStarted, allAnswersIn: _allAnswersIn, roundIndex, difficulty, points, theme: _theme }: TvPlayProps) {
  ...
  return (
    <div className="min-h-dvh flex flex-col p-6 lg:p-10 relative overflow-hidden">
      {/* Animated bomb in corner */}
      <motion.div
        className="absolute top-8 right-8 text-6xl lg:text-7xl select-none pointer-events-none"
        animate={{
          rotate: [0, -8, 8, -5, 5, 0],
        ...
        <div className="w-full h-2 rounded-full bg-bg-elevated overflow-hidden mb-8">
          <motion.div
            className={`h-full rounded-full transition-colors duration-300 ${
              isCritical ? 'bg-neon-pink' : isUrgent ? 'bg-neon-gold' : 'bg-red-500'
            }`}
            initial={{ width: '100%' }}
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        ...
        <div className="lg:col-span-2 flex flex-col gap-6">
          ...
          {question.image_url && (
            <img src={question.image_url} alt="Question visual" className="rounded-2xl max-h-72 object-contain" />
          )}

          {question.options && (
            <div className="grid grid-cols-2 gap-3 mt-2">
              {question.options.map((option, i) => (
                <div key={i} className="flex items-center gap-3 bg-bg-card shadow-soft rounded-xl px-5 py-4">
        ...
        {/* Player sidebar with bomb warning color */}
