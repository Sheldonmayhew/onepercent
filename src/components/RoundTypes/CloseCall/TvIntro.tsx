...
export default function TvIntro({ roundIndex, totalRounds, difficulty, points, roundName, tagline }: TvIntroProps) {
  ...
  return (
    ...
    >
      ...
      {/* Icon — spotlight animation */}
      <motion.div
        className="text-5xl mb-3"
        initial={{ scale: 0, opacity: 0 }}
        animate={{ scale: 1, opacity: [0, 1, 0.7, 1] }}
        transition={{
          scale: { type: 'spring', stiffness: 260, damping: 18, delay: 0.3 },
      ...
      {/* Round name — orange accent */}
      <motion.h1
        className="font-display text-4xl md:text-5xl font-bold text-orange-400 tracking-wider mb-2"
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ type: 'spring', stiffness: 180, damping: 16, delay: 0.4 }}
      >
        {roundName || 'CLOSE CALL'}
      </motion.h1>
