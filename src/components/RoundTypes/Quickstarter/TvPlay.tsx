...
import type { QuickstarterState } from '../../../roundTypes/definitions/quickstarter';
...
export default function TvPlay({
  ...
  const state = roundState as QuickstarterState;
  const diffColour = getDifficultyColour(difficulty);
  const multipliers: Record<string, number> = state?.multipliers ?? {};
    ...
    <div className="min-h-screen bg-bg-primary flex flex-col">
      ...
      <div className="flex items-center justify-between px-6 py-4">
        <div className="flex items-center gap-3">
          ...
          <span className="font-display text-sm font-bold text-neon-cyan uppercase tracking-wider">
            Quickstarter
          </span>
        </div>
      ...
      <div className="flex flex-1 px-6 py-6 gap-6">
        ...
        {/* Player sidebar with multipliers */}
        <div className="w-72 shrink-0">
          <div className="bg-bg-card shadow-soft rounded-2xl p-4">
            ...
            <div className="space-y-2">
              {players.map((player) => {
                const mult = multipliers[player.id] ?? 1;
                const multLabel = mult === 1 ? '1x' : `${mult}x`;
                return (
                  <motion.div
                    key={player.id}
                  ...
                  >
                    ...
                    <span
                      className={`font-score text-xs font-bold px-2 py-0.5 rounded-full ${
                        mult >= 2
